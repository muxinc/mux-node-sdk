...
export const handler = async (client: Mux, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.video.playback.storyboardMeta(PLAYBACK_ID, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
