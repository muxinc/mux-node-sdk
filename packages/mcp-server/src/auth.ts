...
export const parseClientAuthHeaders = (req: IncomingMessage, required?: boolean): Partial<ClientOptions> => { if (req.headers.authorization) {
  ...
  switch (scheme) {
    case 'Basic':
     ...
     return { tokenId: rawValue.slice(0, rawValue.search(':')), tokenSecret: rawValue.slice(rawValue.search(':') + 1) }
    case 'Bearer':
...
const tokenId = Array.isArray(req.headers['x-mux-token-id']) ? req.headers['x-mux-token-id'][0] : req.headers['x-mux-token-id']
const tokenSecret = Array.isArray(req.headers['x-mux-token-secret']) ? req.headers['x-mux-token-secret'][0] : req.headers['x-mux-token-secret']
const authorizationToken = Array.isArray(req.headers['x-mux-authorization-token']) ? req.headers['x-mux-authorization-token'][0] : req.headers['x-mux-authorization-token']
