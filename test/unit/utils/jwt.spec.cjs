...
describe('Utils::JWT', () => {
  ...
  describe('signViewerCounts', () => {
    ...
    it('defaults to rt and includes an expiration', () => {
      ...
      const token = JWT.signViewerCounts('some-space-id', options);
      expect(token).to.be.a('string');
      const decoded = JWT.decode(token);
    ...
    it('accepts a timestamp or time shorthand', () => {
      ...
      const token1 = JWT.signViewerCounts('some-space-id', options1);
      const token2 = JWT.signViewerCounts('some-space-id', options2);
      expect(token1).to.be.a('string');
      expect(token2).to.be.a('string');
